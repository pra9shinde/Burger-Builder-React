...
class App extends Component {
  ...
  deleteCharHandler = (index) => {
    ...
    let charList = userInput.split('');

    charList.splice(index, 1); // Remove Character from arr

    const updatedInput = charList.join('');
    this.setState({
  ...
  render() {
    const charList = this.state.userInput.split('').map((ch, index) => {
      return <Char character={ch} key={index} delete={() => this.deleteCharHandler(index)} />;
    });
